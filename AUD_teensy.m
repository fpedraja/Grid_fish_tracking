...
%%
blocksize=20; numblocks=round(length(files2)/blocksize);
a=1;  Xamp=[]; EODtiend2=[]; 
for o=1:3%numblocks
    Data=[]; Datazs=[]; 
    for i=a:a+19
    ...
    for j=1:size(Data,2)
        ...
        [~,sample1_M1]=findpeaks(Datazs(:,j) ,'MINPEAKHEIGHT',1.4,'MINPEAKDISTANCE',70);
    ...
    EODtiend2=[EODtiend2; EODtiend+(blocksize*Fs*(o-1)*60)];
    Data=[]; Datazs=[]; EODti=[];
    a=a+20;
end
%% identity 
...
figure;
for h=1:max(IDX)
    AUX=diff(EODtiend2(IDX==h)); AUX2=(1./AUX)*Fs; AUX2(end+1)=AUX2(end);
    plot(EODtiend2(IDX==h),AUX2,'.'); hold on;
